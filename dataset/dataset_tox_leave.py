...
import scopy
import scopy.ScoTox
from scopy.ScoTox.tox_filter import Check_Toxicophores
...
def check_tox(mol):
    ...
    #     mol - mol file
    # Outputs:
    #     tox_substr: list of tuples - number of atoms which generate toxicophore

    tox_substr = []
    filter_result = Check_Toxicophores(mol, detail=True, showSMILES=False)
    pattern = filter_result.get('MatchedAtoms')
    #     print(pattern)

    if pattern != ['-']:

        for i, atoms in enumerate(pattern):
            tox_substr.append((atoms[0]))

        return tox_substr


def mask_tox(Graph, tox_substr, mask=False):
    G = Graph.copy()
    substr_num = len(tox_substr)
    removed = []
    atoms_set = []

    if mask:  # remove toxicophoric substructure
        # print('masking toxicophoric substructure')
        for i, substr_atoms in enumerate(tox_substr):
            for n in substr_atoms:
    ...
    else:  # leave toxicophoric substructure
        # print('leaving toxicophoric substructure')
        for n in G.nodes():
            if n not in substr_atoms:
                G.remove_node(n)
                removed.append(n)
...
def remove_subgraph(Graph, center, percent=0.2):
    ...
    while len(removed) < num:
        ...
        for n in temp:
            neighbors.extend([i for i in G.neighbors(n) if i not in temp])  # get neighbors of center atom
        for n in temp:
            if len(removed) < num:  # remove these neighbors
                G.remove_node(n)
                removed.append(n)
            else:
                break
...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, index, mask=True):
        ...
        # Get toxic substructure forming atoms number
        tox_substr = check_tox(mol)

        if tox_substr:

            # Get the graph for i and j after removing/leaving toxic subgraphs
            G_i, removed_i = mask_tox(molGraph, tox_substr, mask=mask)
            G_j, removed_j = mask_tox(molGraph, tox_substr, mask=mask)
            ...
            G_j, removed_j = remove_subgraph(molGraph, start_i, percent=percent_i)
